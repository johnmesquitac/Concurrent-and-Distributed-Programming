...
int main()
{
  ...
  printf("%d, %f\n", tmili, (float)tmili/tfinal);//Tempo paralelo, porcentagem
  printf("%d\n", tnp+tmili);//Tempo total
...
double ComputeForces( Particle myparticles[], Particle others[], ParticleV pv[], int npart )
{
 ...
 #pragma omp parallel num_threads(MAX_THREADS) private(xi, yi, rx, ry, mj, r, fx, fy)
  #pragma omp for schedule(auto) nowait
  for (i=0; i<npart; i++) {
    rmin = 100.0;
    xi   = myparticles[i].x;
...
double ComputeNewPos( Particle particles[], ParticleV pv[], int npart, double max_f)
{
  ...
  a1	 = -(a0 + a2);
 #pragma omp parallel num_threads(MAX_THREADS) private(xi, yi,a0, a1, a2)
 #pragma omp for schedule(auto) nowait
  for (i=0; i<npart; i++) {
